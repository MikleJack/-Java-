<template>
  <div class="max" style="font-size: 20px">
...
export default {
  ...
  methods:{
    ...
    // logging(){
    //   this.$axios.get("http://116.62.218.244:8081/admin/login?name=" + this.ruleForm.admin + "&password=" + this.ruleForm.password).then((res)=>{
    //     if(res.data){
    //       //   $message消息提示框
    //       this.$message({
    //         message: '登录成功',
    ...
    //         center: true
    //       });
    //       this.$axios.get("http://116.62.218.244:8081/admin/selectByname?name="+this.ruleForm.admin).then((res)=>{
    //         if(res.data){
    ...
    // },
    logging(){
      if (this.code == sessionStorage.getItem("vertifyCode")){
        sessionStorage.setItem("token", 'true');
