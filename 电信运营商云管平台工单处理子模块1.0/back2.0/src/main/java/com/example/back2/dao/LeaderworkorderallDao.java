package com.example.back2.dao;

import com.example.back2.entity.Leaderworkorderall;
import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param;

import java.util.List;

/**
 * (Leaderworkorderall)表数据库访问层
 *
 * @author makejava
 * @since 2022-01-10 20:56:41
 */
@Mapper
public interface LeaderworkorderallDao {

    /**
     * 查询指定行数据
     *
     * @param offset 查询起始位置
     * @param limit  查询条数
     * @return 对象列表
     */
    List<Leaderworkorderall> queryAllByLimit(@Param("offset") int offset, @Param("limit") int limit);


    /**
     * 通过实体作为筛选条件查询
     *
     * @param leaderworkorderall 实例对象
     * @return 对象列表
     */
    List<Leaderworkorderall> queryAll(Leaderworkorderall leaderworkorderall);

    /**
     * 新增数据
     *
     * @param leaderworkorderall 实例对象
     * @return 影响行数
     */
    int insert(Leaderworkorderall leaderworkorderall);


//  查询所有的工单
    List<Leaderworkorderall> selectAll();

}

